...
Inherently, single-cell RNA sequencing comes with technical confounders that if not accounted for can impact the interpreation of downstream analysis. 
...
If we were to proceed with downstream analysis, such as differential gene expression and cell clustering, these technical problems would impact the overall conclusions.

For example, let's consider two cells with different sequencing depths. 
Cell A has a total of 20,000 counts and 10,000 of those counts come from gene X and cell B has only 10,000 counts but 5,000 counts come from gene X. 
...
The most commonly used approaches for normalization of single-cell RNA seq data are based on library scaling, where each count is scaled by a cell-specific scaling factor ([Anders and Huber 2010](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2010-11-10-r106)).

The most basic library scaling methods use the assumption that the only difference between each cell is the total number of RNA molecules and that any other technical factors like changes in PCR amplification or mRNA capture efficiency affect each cell equally.
...
1. Calculate the geometric mean of each gene across all cells. 
...
With the high dropout and low counts found in single-cell RNA seq datasets and reliance on balance of differential gene expression, this method does not account for the hetereogeneity commonly present in single-cell RNA seq data. 
Therefore, it should be used with caution as it can impact interpreation of downstream analysis. 
...
Using this method helps account for any bias due to presence of differing cell types or cell states within a single-cell RNA seq dataset that could impact gene expression. 

### Log Transformation

Finally, the normalized counts are computed by dividing the raw counts for each gene by the respective size factor for that cell.
...
- Reduces general skewness of the data to fit the assumptions of downstream tools 
...
We will go ahead and use the `scater::librarySizeFactors` function to compute the size factors for our counts matrix.
...
Rather than use `calculatePCA()` though we can use `runPCA()` and add the information directly to our two `SingleCellExperiments`. 
...
As you can see the normalized matrix with library scaling in comparison to the second plot where we have performed clustering prior to calculating size factors results in a different PCA plot. 
